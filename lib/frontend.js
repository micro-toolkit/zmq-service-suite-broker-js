(function() {
  ...
  var Frontend = function(configuration, smiService){
    ...
    var reply = function(message){
      ...
      log.info("frontend replied to: %s with status: %s", message.identity, message.status);
    ...
    var replyFrames = function(frames){
      ...
      log.info("frontend replied to: %s with status: %s", identity, status);
    ...
    var onMessage = function(){
      ...
      try{
        ...
        log.info("frontend received from: %s", from);
        ...
        if (to === null) {
          ...
          log.error("frontend received an invalid address => %j", msg.address);
          replyError(404, msg);
          return;
        }
